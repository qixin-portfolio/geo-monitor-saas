...
### 6.4 v0.1.4 / Stage 2.4 状态流与人工推进

v0.1.4 在 RepairTask Detail 页新增只读的“状态流与下一步动作”模块，目标是让运营知道当前任务处于哪个阶段、下一步建议做什么、哪些动作必须人工确认，以及什么时候可以进入复测或老板报告。

本轮只做 ViewModel 映射和展示，不写入状态。

现有 DB 状态仍使用 `GeoContentTaskStatus`：
...
产品层状态流使用兼容映射，不改 schema：

| 状态 | 中文名 | 含义 |
| --- | --- | --- |
| `DRAFT` | 草稿 | 任务刚进入修复池，还需要确认证据、风险和建议产出物 |
| `READY` | 可执行 | 可进入人工内容制作，但不代表自动执行 |
| `NEEDS_REVIEW` | 需审核 | 需要补证据或负责人确认后再推进 |
| `IN_PROGRESS` | 处理中 | 已进入人工处理阶段 |
| `PUBLISHED` | 已发布 | 内容已完成发布或导出记录，可准备复测 |
| `RETEST_PENDING` | 待复测 | 等待人工使用同一 query 做复测 |
| `IMPROVED` | 复测改善 | 未来真实复测改善后的展示状态 |
| `NO_CHANGE` | 复测暂无变化 | 未来真实复测暂无变化后的展示状态 |
| `BLOCKED` | 阻塞 | 状态未知或信息不足，不能直接推进 |
| `REJECTED` | 已拒绝 | 已跳过或拒绝，不应继续执行 |

Human Gate 规则：

- `GREEN` 且可执行状态可以进入人工内容制作，但仍需确认事实来源。
- `YELLOW` 必须补证据或人工确认后再处理。
- `RED` 禁止直接执行，只能记录风险或改写方向。
- 未知 / 缺失状态统一视为 `BLOCKED`，不能渲染成可执行状态。
...
后续如果要做“人工状态切换”，需要单独 PRD，并补齐 tenant-scoped update、审核人 / 时间 / 原因、audit log、Staging QA 和 Human Gate。
