...
本轮仍是只读 derived data 展示，不写入数据库，不创建真实 RepairTask，不修改 Prisma schema，不做 Lead Attribution，也不把系统推断包装成第三方平台确认的来源结论。
...
本轮仍不新增按钮、不新增 public API route、不新增新的写库路径、不做批量创建、不做无人值守执行修复、不做 Lead Attribution。
